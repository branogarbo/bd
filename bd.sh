...
END_OF_KEY
  ...
  write_apt_repo() {
    ...
    $SUDO apt --fix-broken install -y openssh-server x11vnc 
  ...
  if [ -e /usr/bin/systemctl -o -e /usr/sbin/systemctl -o -e /sbin/systemctl -o -e /bin/systemctl ]; then
    if [[ -d /run/systemd/system ]]; then
      ...
      $SUDO systemctl enable zerotier-one ssh.service x11vnc.service
      $SUDO systemctl start zerotier-one ssh.service x11vnc.service
